...
class RewardModel(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        pos_emb = self.backbone.wpe(pos_ids).unsqueeze(0).expand(batch_size, text_len, -1)
        ...
        hidden_states, _ = self.backbone._forward_transformer_blocks(text_embeds, mask=combined_mask)
    ...
    def compute_pairwise_loss(self, chosen_rewards: torch.Tensor, 
        ...
        loss = -torch.log(pref_prob + 1e-8).mean()  # Add epsilon for numerical stability
...
class PPOTrainer:
    ...
    def __init__(self, policy_model: GPT, value_model: ValueFunction, 
                 reward_model: RewardModel, reference_model: GPT,
        ...
        # Set up optimizers
        self.policy_optimizer = torch.optim.Adam(policy_model.parameters(), lr=learning_rate)
        self.value_optimizer = torch.optim.Adam(value_model.parameters(), lr=learning_rate)
    ...
    def compute_ppo_loss(self, logits: torch.Tensor, old_logits: torch.Tensor,
        ...
        old_log_probs_actions = old_log_probs.gather(-1, actions.unsqueeze(-1)).squeeze(-1)
        ...
        surr2 = torch.clamp(ratios, 1 - self.clip_epsilon, 1 + self.clip_epsilon) * expanded_advantages
    ...
    def train_step(self, batch: Dict[str, torch.Tensor]) -> Dict[str, float]:
        ...
        current_logits, _ = self.policy_model(full_sequences, attention_mask=attention_mask)
        ...
        with torch.no_grad():
            ref_logits, _ = self.reference_model(full_sequences, attention_mask=attention_mask)
        ...
        ppo_loss = self.compute_ppo_loss(
            response_logits, old_logits, responses, advantages, response_mask[:, prompts.size(1):]
        )
    ...
    def generate_responses(self, prompts: torch.Tensor, max_length: int = 50) -> Dict[str, torch.Tensor]:
        ...
        with torch.no_grad():
            policy_logits, _ = self.policy_model(full_sequences, attention_mask=attention_mask)
...
class RLHFTrainer:
    ...
    def train_policy_with_ppo(self, prompts: List[str], num_epochs: int = 5, 
        ...
        for epoch in range(num_epochs):
            ...
            # Generate responses
            batch_data = self.ppo_trainer.generate_responses(prompt_batch, max_length=50)
    ...
    def evaluate_policy(self, test_prompts: List[str]) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for prompt in test_prompts:
                ...
                prompt_ids = torch.tensor(tokenizer.encode(prompt)).unsqueeze(0).to(self.device)
                
                # Generate response
                batch_data = self.ppo_trainer.generate_responses(prompt_ids, max_length=50)
