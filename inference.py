...
from data import TokenizerInfo, build_tokenizer
...
def prepare_prompt(tokenizer: TokenizerInfo, prompt: str, device: torch.device) -> torch.Tensor:
...
def main() -> None:
    ...
    completion = generate_text(model, tokenizer, prompt, max_new_tokens=120, temperature=0.8)
