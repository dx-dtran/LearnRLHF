...
Theory packet for Part 1 of the crash course. Read through it once
before starting, then re-read the relevant section as you go. The
goal of Part 1 is to be able to draw every arrow, forward and backward, of
GPT-2 without looking anything up.

GPT-2 is a neural network that takes a sequence of tokens and predicts the
next token. It is built from a small set of building blocks (embeddings,
LayerNorm, multi-head self-attention, MLP) stacked many times. The same block
shape repeats from the bottom of the model to the top. Part 1 implements
each block once, gradient-checks it, and then stacks them into the full
model. Section 1 below introduces the high-level structure; sections 2–9
walk through each component in the order they appear in `model.py`.
...
A useful picture to hold while reading: GPT-2 maintains a 2D table with one
row per token position and one column per hidden feature. Embeddings (§2)
fill in the first version of that table from the input ids. Each transformer
block (§4–§6) reads the current table and writes an updated version: the
attention sub-layer copies information from earlier rows into the current
row, and the MLP sub-layer transforms each row on its own. LayerNorm (§3)
keeps the numbers in each row at a stable scale so the next block sees
well-behaved inputs. The final linear layer (§7) turns each row of the last
table into a probability distribution over the next token.

---

## 1. Big picture

GPT-2 is a **decoder-only causal transformer language model**. Three pieces
of jargon, each defined inline:

- **Decoder-only** means the model has only one stack of transformer blocks,
  not two. (The original 2017 Transformer paper had an encoder stack that
  read the input sentence and a decoder stack that produced the translation;
  GPT-2 keeps only the decoder stack and feeds it both prompt and response
  in the same token stream.)
- **Causal** means a token at position `t` can only see tokens at positions
  `0, 1, ..., t` and cannot peek at the future. The "causal mask" inside
  attention is what enforces this.
- **Language model** means the training objective is next-token prediction:
  given the first `t` tokens, predict the token at position `t+1`.
...
**pre-LN**. The original 2017 Transformer paper applied LayerNorm after the
addition (post-LN); pre-LN turned out to be more stable when many layers
are stacked, and GPT-2 (along with every modern decoder) uses it.

The tensor `x` that flows through this stack has a name: the **residual
stream**. Every block reads the residual stream, computes an update, and
adds the update back in. Attention writes information gathered from
previous tokens; the MLP writes per-token nonlinear feature
transformations. The residual addition (`x = x + ...`) keeps the previous
contents of the stream available alongside the new contribution, instead
of replacing them.

---

## 2. Embeddings (provided)
...
## 3. LayerNorm (provided: `ManualLayerNorm` in `model.py`)
...
useful scale and offset *after* normalization. The order matters: the
hidden vector is first standardized to a fixed reference scale, and then
the learned `gamma` and `beta` rescale and shift it back. Without the
standardization step, deep networks can accumulate "activation drift"
where the magnitude of the hidden vector grows or shrinks layer by
layer. A channel whose variance doubles at each block produces a
numerical blow-up 12 blocks later. LayerNorm resets that scale at the
start of every block so the next block always sees a well-behaved input.
...
The provided `ManualLayerNorm` is LayerNorm written from scratch and
...
## 4. Causal self-attention ([FILL 1.1])

Attention is the operation that lets a token at position `t` look at
earlier tokens and pull in whatever information it needs from them. It is
the only sub-layer in a transformer block where information moves between
positions; the MLP and LayerNorm both work one position at a time. The
construction in §4.1–§4.7 builds attention from scratch in seven small
steps; §4.9 and §4.10 walk numerical examples through those steps.
...
for free. The tests for [FILL 1.1] still gradient-check the whole block
at fp64 on a small tensor.
...
Step 5: head 1 runs the same five steps in parallel with its own Q, K,
and V. Concatenate the per-head outputs (each of shape `(T, d_h)`)
along the feature axis to recover a `(T, C)` tensor, project through
`W_o`, and add the result to the residual stream.
...
## 7. Full GPT-2 and weight tying ([FILL 1.2])
...
## 8. Loading HuggingFace weights (provided)
...
A KV cache is added later in Part 4 *only if* PPO rollouts are
...
After finishing Part 1, add the following to this file:

- The HF-to-yours parameter name-map table from the provided weight loader.
