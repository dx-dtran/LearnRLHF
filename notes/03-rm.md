...
Theory packet for Part 3 of the crash course.

In Part 2 the model learned to imitate good-looking assistant replies. SFT
teaches the model what *plausible* answers look like, but it cannot rank two
plausible answers against each other; nothing in the SFT loss says "answer A
is better than answer B." Part 3 adds that ranking.

The plan is to train a second neural network, the **reward model** (RM), that
takes a full (prompt, response) pair and outputs a single scalar score. The
RM is trained on the human preference data already present in HH-RLHF: each
training example is a pair of completions to the same prompt, where a human
labeler has marked one as preferred. The RM learns to give the preferred
completion a higher score than the other one.

Part 4 (PPO) then uses this scalar score as a reward signal: the PPO
policy will be pushed to produce responses the RM scores highly. That makes
RM quality the ceiling on PPO quality. If the RM is biased, the PPO policy
learns to exploit the bias instead of becoming genuinely better.
...
If $r(y_c) \gg r(y_r)$, the sigmoid argument is very positive and $\sigma \to 1$, so the human almost always picks the chosen one. If the two scores
are equal, $\sigma(0) = 0.5$, a coin flip. If $r(y_c) \ll r(y_r)$, $\sigma \to 0$, meaning the human rarely picks the labeled-chosen one (the labeler
...
The RM uses a GPT-2 backbone (same architecture as Part 1) with one extra
...
## 5. Architecture details ([FILL 3.1])

### 5.1 Initialization: from SFT, not base

The RM is the `GPT` model from Part 1 with an extra `nn.Linear(n_embd,
...
## 6. Preference dataset (provided in `data_hh.py`)
...
## 7. Training loop (provided in `train_rm.py`)
...
## 8. Reward calibration (optional exercise)
...
- **A gameable RM gets gamed.** The PPO policy is a powerful function
  ...
  specific phrases), the policy will find and exploit it. Part 5
